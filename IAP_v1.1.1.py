...
#v1.1.1 update includes scientist's initials extractions and better date formatting
...
# Function to handle initials followed by a date in YYYYMMDD format
...
# Extended function to handle cases like "CA16Aug24" and other date formats
def extract_initials_and_date_v2(text):
    # Relaxed pattern to catch initials followed by a date (with or without spaces)
    match = re.search(r'([A-Z]{2})\s*(\d{1,2}[a-zA-Z]{3}\d{2,4})', text)  # Handles "CA16Aug24"
    if match:
        ...
        try:
            day = date_str[:2]
            month = date_str[2:5]
            year_yy = int(date_str[-2:])
            # Convert two-digit year to four-digit year
            if year_yy < 50:  # Assuming the year is after 1950, so anything less than 50 is 2000s
                year_full = 2000 + year_yy
            else:
                year_full = 1900 + year_yy
            # Manually reconstruct the date in YYYY-MM-DD format
            formatted_date = f"{year_full}-{datetime.strptime(month, '%b').strftime('%m')}-{day}"
            return initials, formatted_date
        except ValueError as e:
            return initials, None  # Return initials even if date fails to parse

    return extract_initials_and_date(text)

# Combined logic to handle all date formats including "1DEC2022", "CA16Aug24", etc.
def extract_and_format_dates_v7(text):
    initials, formatted_date = extract_initials_and_date_v2(text)
...
def extract_final_two_letter_initials_v4(text):
    initials, _ = extract_initials_and_date_v2(text)
    if initials:
        return initials
...
file_name = 'CRNX LN2 Inventory GONKY.xlsx'  # Replace with the correct file path
        ...
        for row in range(9):
            for col in range(9):
                ...
                if pd.notna(value):  # Only include non-empty cells
                    ...
                    formatted_dates = extract_and_format_dates_v7(str(value))
                    # Extract two-letter initials, handling stars and special cases like "PA 20240805"
                    initials = extract_final_two_letter_initials_v4(str(value))
...
output_file = os.path.join(current_dir, "IAPv1.1.1_GONKY_Output.xlsx")
