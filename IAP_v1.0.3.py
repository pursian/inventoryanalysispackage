#Inventory Analysis Package v1.0.3 (IAP)
#Ehsun Yazdani, Crinetics Pharmaceuticals
#v1.0.3 update includes scientist's initials extractions and better date formatting
...
# Function to extract two-letter initials, allowing for trailing spaces
def extract_initials_with_space(text):
    match = re.search(r'([A-Z]{2})\s*$', text)  # Look for two uppercase letters followed by space
        ...
        for row in range(9):
            for col in range(9):
                ...
                if pd.notna(value):  # Only include non-empty cells
                    ...
                    # Extract initials with handling for trailing spaces
                    initials = extract_initials_with_space(str(value))
                    data.append([dewar, rack, box, index, value, formatted_dates, initials])

# Convert the collected data into a DataFrame
output_df = pd.DataFrame(data, columns=["Dewar", "Rack", "Box", "Tile Index", "Contents", "Formatted Dates", "Initials"])

# Save to a new Excel file
output_file = os.path.join(current_dir, "IAPv1.0.3_GONKY_Output.xlsx")
